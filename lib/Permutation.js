...
function checkExpected( actual, expected, emitter ) {
	if ( actual === expected ) {
		...
		var err = new Error( 'The expected amount of permutation was not reached. Expected: ' + expectedPermutations );
    ...
    var get = function () {
		...
		if ( options.max !== undefined && options.max !== null )  {
			if ( typeof options.max !== 'number' ) {
				var err = new Error( 'The "options.max" value must be a number' );
				emitter.emit( 'error', err );
				return;
			} else if ( options.max < 0 ) {
				var err = new Error( 'The "options.max" value must be a positive number' );
				emitter.emit( 'error', err );
				return;
			} else if ( options.max === 0 ) {
				emitter.emit( 'end' );
