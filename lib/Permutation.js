...
var  {factorial, defaultComparator} = require('./utils');
 ...
 */
function checkExpected(actual, expectedTester, emitter) {
...
function emitSmallPermutation(data, expectedPermutationTester, emitter) {
...
function Permutation(data, comparator, opt) {
	...
	var emitter = this;
	var expectedPermutationTester;
	var isMaxPermutationReached;
	if (options.max !== undefined && options.max !== null) {
		//expectedPermutations = options.max;
		expectedPermutationTester = function ( actual ) {
			return actual <= options.max;
		};
		isMaxPermutationReached =  function ( actual ) {
			return actual >= options.max;
		};
	} else {
	...
	if (emitSmallPermutation(data, expectedPermutationTester, emitter)) {
		return;
	}
...
//			} else {
	...
	var get = function () {
		...
		if ( isMaxPermutationReached( actualPermutationFound ) ) {
			return;
		}
		...
		while (true) {
			...
			if ( isMaxPermutationReached( actualPermutationFound ) || interrupted) {
				break;
			}
		...
		if (!interrupted) {
			checkExpected(actualPermutationFound, expectedPermutationTester, emitter);
		}
	};
